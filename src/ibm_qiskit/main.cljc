(ns ibm-qiskit.main
  "Kotodama WASM entrypoint for the IBM Qiskit clean-room actor (L4) — Clojure port.
  ...
  facts (`ibm_qiskit.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ibm_qiskit")
(def tier "L4")
...
(def entity-specs
  [{:entity "Job"        :plural "jobs"        :id-prefix "ibmqiski_job"
    :fields [:jobId :backend :status]                                        :required [:backend :status]
    :coerce {}                                                                :refs {}}
   {:entity "RuntimeJob" :plural "runtimejobs" :id-prefix "ibmqiski_run"
    :fields [:jobId :programId :creationDate :image :sessionId :version :private :status] :required [:creationDate :image]
    :coerce {:version :int :private :bool}                                    :refs {:jobId "Job"}}
   {:entity "Backend"    :plural "backends"    :id-prefix "ibmqiski_bac"
    :fields [:name :backendVersion :numQubits :simulator :local :conditional :openPulse :memory :dt :dtm] :required [:name :backendVersion]
    :coerce {:numQubits :int :simulator :bool :local :bool :conditional :bool :openPulse :bool :memory :bool :dt :float :dtm :float} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"ibm_qiskit.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ibm_qiskit-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ibm_qiskit-compat/ready)
